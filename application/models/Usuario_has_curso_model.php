...
class Usuario_has_curso_model extends MY_Model
{
    ...
    public function UsuariosCurso($pin)
    {
        if(is_null($pin))
            return false;
        $this->db->select('Usuario_RA, Usuario.Nome as Nome');
        $this->db->join('Usuario','Usuario_RA = RA');
        $this->db->where('Curso_PIN', $pin);
        $this->db->where('Usuario.Tipo_Usuario !=','1');
        $query = $this->db->get($this->table);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
    ...
    public function UsuariosCursoLeaderboard($pin){
        ...
        $sql = "SELECT Usuario.Nome, Usuario_has_Curso.Pontuacao, COUNT(Usuario_has_Medalha.idUsuario_has_medalha) as Qtd_Medalha, COUNT(Usuario_has_Trofeu.idUsuario_has_Trofeu) as Qtd_Trofeu, Usuario.XP\n"
